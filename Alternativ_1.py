...
def handle_engineer_action(bot: dict, current_round: int, balance: int) -> dict:
    ...
    # В перші кроки лупашимо вітряки незалежно від місця локалізації
...
def get_bot_action(bot_id: int) -> Tuple[dict, int]:
    ...
    if bot_type == 'FACTORY':
        ...
        action = handle_engineer_action(bot, current_round, balance)
    else:
...
@app.route('/agent/<int:bot_id>/wiev', methods=['POST'])
